...
sw_split_name <- starwars %>% 
  separate(col = name, 
           ...
           sep = "-")
## in row 49 the characters after the second dash (Mundi) are dropped 
```
...
mpg_model_class <- mpg %>% # Start with mpg
  unite(col = model_class, # Create a new column called 'model_class'
        c(model, class), # That combines the manufacturer & model column
        sep = "hey?"  # Puts "hey?" between the pieces (default is underscore)
        ) 
```
...
cali_dates <- c(03032020, 09091998,08181900)
cali_dates_converted <- mdy(cali_dates)
cali_dates_converted
